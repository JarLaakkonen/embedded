/*
                     HUOMIO Opelle!
        Tässä on 3 pisteen suoritus 12/10/2025
        Tiedostossa löytyy kuva(Time-RGY_Parse_test_tulos) jossa näkyy onnistunut Google test case 3 pisteen suorituksena.
        Koodi löytyy Test code kansion /test_cases sisältä jos haluatte käydä tutkimassa tarkemmin(Koodista on myös kuva jossa näkyy suurinpiirtein sen koodi.(Kuva_1))
        Niissä käy ilmi, että 3 pisteen vaatimukset täyttyy google testin mukaan, lisäsin testikeissejä
        ja lisätty  test caset R G Y arvoja varten.

        Olen myös lisännyt tähän koodiin toiminnot jolla näky errori koodit.
        Kuvassa Koodin_testaus_2p näkyy test casen ulostulot ja annettut arvot
        eli siis vääriä arvoja syötin konsoliin ja yhden oikean ja se huomasi jokaisen.

        Sama homma kuvassa RGY_RTO_Testaus' kuvassa näkyy sama, mutta se hyväksyy vain arvot jotka ovat R G tai Y.

        Koodi tällä hetkellä katsoo ensimmäisen buffer arvon mukaan kumman testi case mukaan se menee.
        tämä sen takia että voidaan ajaa molemmat helposti tässä koodissa.

         LYHYESTI mitä tässä koodissa siis tapahtuu.
         Alustetaan kaikki tarvittavat ensin(fifo valot uart jne)
         ja odotetaan että uarttiin tulee jotain.
         kun konsoliin kirjoitetaan ja painetaan enter, koodi katsoo ensin alkaako arvo 0(tämä sen takia että voidaan tarkistaa HHMMSS).
         Jos alkaa niin vertaillaan onko muoto oikea. jos ei mennään error_taskiin ja printataa error. Jos on tulostetaan arvo sekuntteina
         
         Kun arvo ei ala 0 niin se lähetetään light_parse ja jos se on oikeassa muodossa niin valoja vaihdellaan sen mukaan, jos ei niin taas tulee error.

         Jos selitys ei riitä niin voin tottakai kirjoittaa tarkemmin,mutta eiköhän tässä ole riittävästi luettavaa jo.
*/
...
#define TIME_SECONDS_ZERO_ERROR      -7
